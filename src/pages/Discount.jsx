...
export default function Discount() {
  ...
  const fetchDiscount = async () => {
    try {
      ...
      setDiscount(response?.data?.data); // Adjust based on actual API structure
      console.log("Discounts fetched:", response?.data?.data);
    } catch (err) {
  ...
  return (
    <div className="overflow-x-auto px-4 py-6">
      <div className="flex justify-between mb-4">
        <h2 className="text-2xl font-semibold text-gray-800">Discounts</h2>
      ...
      <table className="min-w-full bg-white rounded-xl shadow-md overflow-hidden">
        <thead className="bg-blue-600 text-white">
          <tr>
            ...
            <th className="text-left px-6 py-3">Finished Date</th>
          ...
          {discount && discount.length > 0 ? (
            discount.map((item, index) => (
              <tr key={item.id} className="hover:bg-gray-100 border-b">
                ...
                <td className="px-6 py-4">{item?.started_at}</td>
                <td className="px-6 py-4">{item?.finished_at}</td>
                <td
                  className={`px-6 py-4 font-medium ${item?.status ? 'text-green-600' : 'text-red-600'
                    }`}
                >
